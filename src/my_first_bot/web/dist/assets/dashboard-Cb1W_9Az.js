import"./modulepreload-polyfill-B5Qt9EMX.js";import{a as X,b as J,c as K,d as Q}from"./api-C-_1ynU0.js";const m=i=>String(i).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),Z=i=>i.vehicle_id==="my_robot"?`${m(i.vehicle_id)} <span class="tag">AMR</span>`:m(i.vehicle_id),H={moving:["Moving","moving"],turning:["Turning normally","turning"],waiting_vehicle:["Waiting for vehicle","waiting"],blocked_obstacle:["Blocked by obstacle","blocked"],stalled:["Commanded but not moving","blocked"],stuck:["Stuck","blocked"],idle:["Idle / intentional stop","idle"],planning:["Planning next route","idle"],localizing:["Waiting for localization","idle"],sensor_wait:["Waiting for LiDAR","idle"],unknown:["State unavailable","idle"]};function W(i){const t=i.speed>=.05?"moving":"unknown",e=i.motion_state||t,[s,a]=H[e]||H.unknown;return{state:e,label:s,className:a}}function tt(i,t){var o;const e=t.latest.filter(c=>W(c).state==="moving").length,s=(o=t.localization)==null?void 0:o.summary,a=s!=null&&s.samples?`${s.mean_error.toFixed(2)} m`:"—";i.innerHTML=[["samples",t.samples.toLocaleString(),"Position samples",""],["vehicles",t.latest.length,"Active vehicles","teal"],["moving",e,"Moving now","teal"],["stuck",t.stuck.length,"Stuck hotspots","orange"],["congestion",t.congestion.length,"Congestion hotspots","red"],["localization",a,"Mean AMCL error","teal"]].map(c=>`<article class="metric ${c[3]}"><strong>${c[1]}</strong><span>${c[2]}</span></article>`).join("")}function et(i,t,e=[]){if(!t.length){i.innerHTML='<div class="empty">No fresh vehicle positions at the end of this range.</div>';return}i.innerHTML=t.map(s=>{const a=W(s),o=e.find(c=>c.vehicle_id===s.vehicle_id);return`<article class="vehicle-card vehicle-action" data-vehicle="${m(s.vehicle_id)}" role="button" tabindex="0" title="Show ${m(s.vehicle_id)} path on the map">
    ...
    <div class="uwb-list">${s.map(u=>{var D;const[f,g]=o[u.state]||["Unknown","unavailable"],x=Number.isFinite(u.error_m)?`${u.error_m.toFixed(2)} m difference`:u.state==="unsynchronized"&&Number.isFinite(u.measurement_skew_s)?`${u.measurement_skew_s.toFixed(2)} s time mismatch`:((D=u.uwb_reason)==null?void 0:D.replaceAll("_"," "))||"No comparison",S=u.raw_state&&u.raw_state!==u.state?` · checking ${u.raw_state.replaceAll("_"," ")}`:"",R=d.get(u.vehicle_id),G=R?` · interlock ${R.state.replaceAll("_"," ")}`:"";return`<div class="uwb-row">
        <span><b>${m(u.vehicle_id)}</b><small>${u.visible_tag_count||0} tags visible · ${m(x)}${m(S)}${m(G)}</small></span>
        <strong class="uwb-state ${g}">${f}</strong>
      </div>`}).join("")}</div>
    <p class="uwb-note">${t.live?"Live comparison":`Recorded comparison${h?` at ${h}`:""}`}. AMCL remains the position source; UWB is confirmation only.</p>`}function at(i,t){const e=t==null?void 0:t.most_stuck_location,s=t==null?void 0:t.worst_path,a=(o,c)=>o?`${new Date(o*1e3).toLocaleString()} → ${new Date((c||o)*1e3).toLocaleString()}`:"No slow/stuck interval in this range";i.innerHTML=`
  ...
  </button>`).join(""):'<div class="empty">No stuck or congestion events.</div>'}function nt(i,t){const e=(t==null?void 0:t.buckets)||[];if(!e.length){i.innerHTML='<div class="empty">No stuck vehicles in this time range.</div>';return}const s=t.bucket_seconds||60,a=s<3600?`${s/60} minute${s===60?"":"s"}`:`${s/3600} hour${s===3600?"":"s"}`;i.innerHTML=`
...
`;const l=i=>document.querySelector(i),y=new ct(l("#map")),w=new rt(l("#routeMap"),l("#routeTooltip")),n={data:null,bounds:null,cursor:null,auto:!0,loading:!1,playing:!1,playbackEnd:null,playbackStart:null,lastTick:0,debounce:null,selectedHotspot:null,selectedVehicle:null,route:null,routeLoading:!1},A={samples:0,latest:[],density:[],tracks:[],stuck:[],stuck_timeline:{bucket_seconds:60,buckets:[]},congestion:[],start:new Date().toISOString(),end:new Date().toISOString(),analytics:{},localization:{summary:{samples:0},vehicles:[]},uwb_validation:{live:!0,summary:{},vehicles:[]},localization_recovery:{live:!0,summary:{},vehicles:[]}};function $(i){return new Date(i.getTime()-i.getTimezoneOffset()*6e4).toISOString().slice(0,19)}function _(i){l("#selectedTime").value=$(new Date(i*1e3))}function L(i){i!==null&&(l("#timeline").value=i,l("#replayClock").textContent=new Date(i*1e3).toLocaleString())}function I(i){return n.bounds?Math.max(n.bounds.first,Math.min(n.bounds.last,i)):i}function j(){return{paths:l("#pathLayer").checked,vehicles:l("#vehicleLayer").checked,heat:l("#densityLayer").checked,heatMetric:l("#heatMetric").value,stuck:l("#stuckLayer").checked,congestion:l("#jamLayer").checked,tags:l("#tagLayer").checked,stateFilter:l("#stateFilter").value,heatFilter:Number(l("#heatFilter").value)/100}}function M(){n.data&&y.draw(n.data,j())}function O(i){var t;if(n.data=i,tt(l("#metrics"),i),et(l("#summaryRows"),i.latest,((t=i.localization)==null?void 0:t.vehicles)||[]),st(l("#localization"),i.localization),it(l("#uwbValidation"),i.uwb_validation,i.localization_recovery),n.selectedVehicle){const e=[...document.querySelectorAll("#summaryRows [data-vehicle]")].find(s=>s.dataset.vehicle===n.selectedVehicle);e&&e.classList.add("selected")}if(at(l("#analytics"),i.analytics),ot(l("#hotspots"),i),nt(l("#stuckTimeline"),i.stuck_timeline),ht(i.latest),n.selectedHotspot){const e=[...document.querySelectorAll("#hotspots [data-hotspot]")].find(s=>Math.abs(Number(s.dataset.x)-n.selectedHotspot.x)<.001&&Math.abs(Number(s.dataset.y)-n.selectedHotspot.y)<.001&&s.dataset.type===n.selectedHotspot.type);e&&e.classList.add("selected")}l("#range").textContent=i.samples?`${new Date(i.start).toLocaleString()} — ${new Date(i.end).toLocaleString()}`:"Waiting for ROS traffic history",M()}function ht(i){const t=l("#routeVehicle"),e=t.value;t.replaceChildren(...i.length?i.map(a=>new Option(a.vehicle_id,a.vehicle_id)):[new Option("No vehicles in this time range","")]);const s=n.selectedVehicle||e;i.some(a=>a.vehicle_id===s)&&(t.value=s)}async function C(i){if(!n.loading){n.loading=!0;try{O(await K(i)),N(!0,`Updated ${new Date().toLocaleTimeString()}`)}catch(t){N(!1,`Monitor offline · ${t.message}`)}finally{n.loading=!1}}}async function z(i=!1){try{const t=await J();if(t.empty){l("#replayClock").textContent="No recorded history";return}const e=n.bounds===null;n.bounds=t,l("#timeline").min=t.first,l("#timeline").max=t.last,l("#selectedTime").min=$(new Date(t.first*1e3)),l("#selectedTime").max=$(new Date(t.last*1e3)),l("#firstLog").textContent=`First ${new Date(t.first*1e3).toLocaleString()}`,l("#lastLog").textContent=`Latest ${new Date(t.last*1e3).toLocaleString()}`,(i||e||!l("#selectedTime").value)&&(n.cursor=t.last,_(Math.max(t.first,t.last-300)),l("#rangeStart").value=$(new Date(t.first*1e3)),l("#rangeEnd").value=$(new Date(t.last*1e3))),(n.cursor===null||n.auto)&&(n.cursor=t.last),L(n.cursor)}catch{l("#replayClock").textContent="History unavailable"}}function b(i="PAUSED"){n.playing=!1,n.lastTick=0,l("#playButton").textContent="▶ Play to latest",l("#playButton").className="",i&&F(i)}function k(i){if(!n.bounds)return;n.auto=!1,n.cursor=I(i),L(n.cursor);const t=Number(l("#trailMinutes").value)*60,e=n.playing&&n.playbackStart!==null?n.playbackStart:Math.max(n.bounds.first,n.cursor-t),s=Math.min(n.cursor-.001,e);C({start:new Date(s*1e3).toISOString(),end:new Date(n.cursor*1e3).toISOString()})}function T(){b(null),n.auto=!0,n.playbackStart=null,F("LIVE"),n.bounds&&(n.cursor=n.bounds.last,L(n.cursor)),C({hours:Number(l("#trailMinutes").value)/60})}function ut(){b("HISTORY");const i=l("#selectedTime").value;i&&k(new Date(i).getTime()/1e3)}function pt(){if(n.playing){b();return}if(!n.bounds)return;n.auto=!1;const i=new Date(l("#selectedTime").value).getTime()/1e3;n.cursor=I(Number.isFinite(i)?i:n.bounds.first),n.playbackStart=n.cursor,n.playbackEnd=n.bounds.last,n.playing=!0,n.lastTick=performance.now(),F("REPLAY"),l("#playButton").textContent="❚❚ Pause replay",l("#playButton").className="playing",k(n.cursor)}function mt(){if(!n.playing)return;const i=performance.now();if(n.loading){n.lastTick=i;return}const t=Math.min(2,(i-n.lastTick)/1e3);n.lastTick=i,n.cursor=Math.min(n.playbackEnd,n.cursor+t*Number(l("#playSpeed").value)),_(n.cursor),k(n.cursor),n.cursor>=n.playbackEnd&&b("HISTORY")}function ft(){const i=l("#rangeStart").value,t=l("#rangeEnd").value;!i||!t||(b("HISTORY"),n.auto=!1,C({start:new Date(i).toISOString(),end:new Date(t).toISOString()}))}l("#liveButton").addEventListener("click",T);l("#jumpButton").addEventListener("click",ut);l("#playButton").addEventListener("click",pt);l("#applyRange").addEventListener("click",ft);l("#stateFilter").addEventListener("change",M);function q(i){var e,s;n.selectedVehicle=i.dataset.vehicle,n.selectedHotspot=null,document.querySelectorAll("#summaryRows [data-vehicle]").forEach(a=>{a.classList.toggle("selected",a===i)}),document.querySelectorAll("#hotspots [data-hotspot]").forEach(a=>a.classList.remove("selected")),l("#pathLayer").checked=!0,y.focusVehicle(n.selectedVehicle),[...l("#routeVehicle").options].some(a=>a.value===n.selectedVehicle)&&(l("#routeVehicle").value=n.selectedVehicle);const t=(s=(e=n.data)==null?void 0:e.latest)==null?void 0:s.find(a=>a.vehicle_id===n.selectedVehicle);l("#mapFocus").textContent=t?`${n.selectedVehicle} path · x ${t.x.toFixed(1)} · y ${t.y.toFixed(1)} · ${t.speed.toFixed(2)} m/s`:`${n.selectedVehicle} path`,l(".map-panel").scrollIntoView({behavior:"smooth",block:"center"})}l("#summaryRows").addEventListener("click",i=>{const t=i.target.closest("[data-vehicle]");t&&q(t)});l("#summaryRows").addEventListener("keydown",i=>{if(i.key!=="Enter"&&i.key!==" ")return;const t=i.target.closest("[data-vehicle]");t&&(i.preventDefault(),q(t))});l("#hotspots").addEventListener("click",i=>{const t=i.target.closest("[data-hotspot]");if(!t)return;n.selectedVehicle=null,l("#pathLayer").checked=!1,document.querySelectorAll("#summaryRows [data-vehicle]").forEach(s=>s.classList.remove("selected")),n.selectedHotspot={x:Number(t.dataset.x),y:Number(t.dataset.y),type:t.dataset.type,events:Number(t.dataset.events),first_started:Number(t.dataset.first)||null,last_ended:Number(t.dataset.last)||null},document.querySelectorAll("#hotspots [data-hotspot]").forEach(s=>{s.classList.toggle("selected",s===t)}),y.focusAt(n.selectedHotspot);const e=n.selectedHotspot.first_started?`${new Date(n.selectedHotspot.first_started*1e3).toLocaleString()} → ${new Date((n.selectedHotspot.last_ended||n.selectedHotspot.first_started)*1e3).toLocaleString()}`:"time unavailable";l("#mapFocus").textContent=`${n.selectedHotspot.type} · x ${n.selectedHotspot.x.toFixed(1)} · y ${n.selectedHotspot.y.toFixed(1)} · ${n.selectedHotspot.events} event(s) · ${e}`,l(".map-panel").scrollIntoView({behavior:"smooth",block:"center"})});l("#stuckTimeline").addEventListener("click",i=>{const t=i.target.closest("[data-stuck-time]");t&&(n.selectedVehicle=null,l("#pathLayer").checked=!1,document.querySelectorAll("#summaryRows [data-vehicle]").forEach(e=>e.classList.remove("selected")),document.querySelectorAll("#hotspots [data-hotspot]").forEach(e=>e.classList.remove("selected")),n.selectedHotspot={x:Number(t.dataset.x),y:Number(t.dataset.y),type:"Stuck",events:Number(t.dataset.count),first_started:Number(t.dataset.time),last_ended:Number(t.dataset.time)},y.focusAt(n.selectedHotspot),_(Number(t.dataset.time)),l("#mapFocus").textContent=`Stuck at ${new Date(Number(t.dataset.time)*1e3).toLocaleTimeString()} · x ${n.selectedHotspot.x.toFixed(1)} · y ${n.selectedHotspot.y.toFixed(1)} · ${n.selectedHotspot.events} vehicle(s)`,l(".map-panel").scrollIntoView({behavior:"smooth",block:"center"}))});function U(i){var t,e;if(i.dataset.insight==="stuck")n.selectedVehicle=null,l("#pathLayer").checked=!1,document.querySelectorAll("#summaryRows [data-vehicle]").forEach(s=>s.classList.remove("selected")),n.selectedHotspot={x:Number(i.dataset.x),y:Number(i.dataset.y),type:"Stuck",events:Number(i.dataset.events),first_started:Number(i.dataset.first)||null,last_ended:Number(i.dataset.last)||null},y.focusAt(n.selectedHotspot),l("#mapFocus").textContent=`Stuck · x ${n.selectedHotspot.x.toFixed(1)} · y ${n.selectedHotspot.y.toFixed(1)} · ${n.selectedHotspot.events} event(s)`;else{const s=i.dataset.vehicle,a=(e=(t=n.data)==null?void 0:t.analytics)==null?void 0:e.worst_path;n.selectedVehicle=s,n.selectedHotspot=null,l("#pathLayer").checked=!0,document.querySelectorAll("#summaryRows [data-vehicle]").forEach(o=>{o.classList.toggle("selected",o.dataset.vehicle===s)}),document.querySelectorAll("#hotspots [data-hotspot]").forEach(o=>o.classList.remove("selected")),y.focusVehicle(s),l("#mapFocus").textContent=`${s} path · ${i.dataset.insight==="window"?"bad interval":"worst path"}${a?` · ${a.slow_seconds.toFixed(0)} s slow`:""}`}l(".map-panel").scrollIntoView({behavior:"smooth",block:"center"})}l("#analytics").addEventListener("click",i=>{const t=i.target.closest("[data-insight]");t&&U(t)});l("#analytics").addEventListener("keydown",i=>{if(i.key!=="Enter"&&i.key!==" ")return;const t=i.target.closest("[data-insight]");t&&(i.preventDefault(),U(t))});["pathLayer","vehicleLayer","densityLayer","stuckLayer","jamLayer","tagLayer","heatMetric","heatFilter"].forEach(i=>l(`#${i}`).addEventListener("input",M));l("#trailMinutes").addEventListener("change",()=>n.auto?T():n.cursor!==null&&k(n.cursor));["selectedTime","rangeStart","rangeEnd"].forEach(i=>l(`#${i}`).addEventListener("focus",()=>{n.auto=!1,b("PAUSED")}));l("#timeline").addEventListener("input",i=>{n.auto=!1,b("HISTORY"),n.cursor=Number(i.target.value),_(n.cursor),L(n.cursor),clearTimeout(n.debounce),n.debounce=setTimeout(()=>k(n.cursor),120)});function Y(){n.route=null,w.setRoute(null),B(l("#routeResult"),null)}w.onDestination(i=>{l("#routeX").value=i.x.toFixed(2),l("#routeY").value=i.y.toFixed(2),l("#routeStatus").className="route-status",l("#routeStatus").textContent=`Destination selected at x ${i.x.toFixed(2)}, y ${i.y.toFixed(2)}. Generate the suggestion when ready.`});l("#routeVehicle").addEventListener("change",Y);l("#routeForm").addEventListener("submit",async i=>{var o;if(i.preventDefault(),n.routeLoading)return;const t=l("#routeVehicle").value,e=Number(l("#routeX").value),s=Number(l("#routeY").value),a=l("#routeStatus");if(!t||!Number.isFinite(e)||!Number.isFinite(s)){a.className="route-status error",a.textContent="Choose a forklift and set both destination coordinates.";return}if(!((o=n.data)!=null&&o.samples)){a.className="route-status error",a.textContent="No recorded traffic is available in this dashboard time range.";return}n.routeLoading=!0,l("#suggestRouteButton").disabled=!0,a.className="route-status loading",a.textContent="Calculating collision-clear alternatives…";try{n.route=await Q({vehicle_id:t,destination:{x:e,y:s},nominal_speed_mps:Number(l("#routeSpeed").value),start:n.data.start,end:n.data.end}),w.setRoute(n.route),B(l("#routeResult"),n.route),a.className="route-status success",a.textContent=`${t}: route ready. Green is the suggested path; dashed gray is the shortest path.`}catch(c){Y(),a.className="route-status error",a.textContent=c.message}finally{n.routeLoading=!1,l("#suggestRouteButton").disabled=!1}});async function gt(){O(A);try{const i=await X();await Promise.all([y.load(i),w.load(i)]),M()}catch{await Promise.all([y.load(y.info,"/fallback-map.png"),w.load(w.info,"/fallback-map.png")]),y.draw(A,j()),N(!1,"Map preview · ROS monitor offline")}await z(!0),T()}gt();setInterval(mt,400);let P=0;setInterval(()=>{n.auto&&T(),P+=1,P%5===0&&z()},2e3);
